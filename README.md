...
### Обновление кэша
Кэши для комплированных проверок и для политик обновляются параллельно (CacheRefreshService).
Обновление запускается как по расписанию (application.yaml - sync раздел), так и по REST API.
Обновление потокобезопасно. Если запущено обновление по расписанию, то по REST API вернется 409 Conflict (уже обновляется). И наоборот, если запущено обновление по REST API, то следующий запуск по расписанию не начнется, пока не завершится ранее запущенное обновление.
Timeout настраивается из application.yaml и является серверным, т.е. при превышении принудительно на сервере принудительно завершается выполнение обновления. Принудительное завершение также безопасно в виду ведения версионности. Если кэш начал обновляться, но при этом не завершил обновление, то при следующем запуске будет создана новая полная версия.

### Расписание обновления кэша
По умолчанию включение обновления по расписанию задается через application.yaml.
В процессе работы приложения можно включить/выключить обновление по расписанию через PUT /cache/scheduled/enable или disable.
